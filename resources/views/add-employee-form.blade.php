<x-app-layout>
    ...
    <div class="mx-w-7xl sm:px-6 lg:px-8 py-4">
        <div class="bg-gray-700 rounded-xl p-5">
            <form method="POST" action="{{ route('employee.store') }}" class="w-full">
                @csrf
                <div class="w-full flex flex-wrap gap-[1%]">
                    ...
                    <div class="mb-3 w-[49%]">
                        <input
                            class="bg-grey-700 border-1 rounded-xl w-full py-2 px-3 bg-gray-700 focus:outline-gray-300 outline-none text-white"
                            placeholder="Employee Last Name" type="text" name="last_name">
                        @error('last_name')
                            <small class="text-red-200">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="mb-3 w-[49%]">
                        <input
                            class="bg-grey-700 border-1 rounded-xl w-full py-2 px-3 bg-gray-700 focus:outline-gray-300 outline-none text-white"
                            placeholder="Employee Email" type="email" name="email">
                        @error('email')
                            <small class="text-red-200">{{ $message }}</small>
                        @enderror
                    </div>
                    <div class="mb-3 w-[49%]">
                        <input
                            class="bg-grey-700 border-1 rounded-xl w-full py-2 px-3 bg-gray-700 focus:outline-gray-300 outline-none text-white"
                            placeholder="Employee Phone" type="text" name="phone">
                    ...
                    <div>
                        <input class="text-white bg-gray-700 py-2 px-3 rounded-xl border border-gray-500 cursor-pointer"
                            type="submit" value="Save Employee Info">
                    </div>
                </div>
            </form>
        </div>
